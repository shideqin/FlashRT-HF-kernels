...
void scaled_fp4_quant(torch::Tensor const& input,
  ...
  if (tensor_layout == 0) {
    ...
    TORCH_CHECK(tensor_layout == 2, "tensor_layout must be 0 (NHD), 1 (HND), or 2 (NHD input/HND output)");
    num_tokens = input.size(1);
    num_heads = input.size(2);
    stride_seq_input = input.stride(1);
    stride_h_input = input.stride(2);
...
void scaled_fp4_quant_permute(torch::Tensor const& input,
  ...
  if (tensor_layout == 0) {
    ...
    stride_seq_output_sf = output_sf.stride(2);
    stride_h_output_sf = output_sf.stride(1);
    const int padded_tokens = ((num_tokens + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
    CHECK_SHAPE(output, batch_size, num_heads, padded_tokens, head_dim / 2);
    CHECK_SHAPE(output_sf, batch_size, num_heads, padded_tokens, head_dim / 16);
  }
...
void scaled_fp4_quant_trans(torch::Tensor const& input,
  ...
  if (tensor_layout == 0) {
    ...
    stride_h_output_sf = output_sf.stride(1);
    const int padded_tokens = ((num_tokens + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
    CHECK_SHAPE(output, batch_size, num_heads, head_dim, padded_tokens / 2);
    CHECK_SHAPE(output_sf, batch_size, num_heads, head_dim, padded_tokens / 16);
  }
