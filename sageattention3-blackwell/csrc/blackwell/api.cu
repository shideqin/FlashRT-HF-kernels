...
void mha_fwd_static(const at::Tensor &q,         // batch_size x num_heads x seqlen_q x (head_size // 2)
    ...
    TORCH_CHECK(seqlen_q % 128 == 0 && seqlen_k % 128 == 0,
                "Q and K sequence storage must be padded to a multiple of 128");
    TORCH_CHECK(unpadded_k > 0 && unpadded_k <= seqlen_k,
                "unpadded_k must be in (0, padded K length]");
    ...
    TORCH_CHECK(out.scalar_type() == dtype, "out has the wrong dtype");
    CHECK_SHAPE(out, batch_size, num_heads, seqlen_q, unpacked_head_size);
    CHECK_DEVICE(out);
    TORCH_CHECK(out.stride(-1) == 1,
                "out head dimension must be contiguous");
    ...
    TORCH_CHECK(softmax_lse.is_contiguous(), "softmax_lse must be contiguous");
    TORCH_CHECK(tile_count_semaphore.scalar_type() == at::kInt,
                "tile_count_semaphore must be int32");
    TORCH_CHECK(tile_count_semaphore.numel() >= 1,
                "tile_count_semaphore must have at least one element");
    CHECK_DEVICE(tile_count_semaphore);
    at::Tensor p;
    ...
    if (is_causal) tile_count_semaphore.fill_(132);
